...
@pytest.fixture
def db(app):
    """Direct DB connection for seeding deterministic data only."""
...
@pytest.fixture
def sample_data(db):
    """Pre-seed deterministic test data. All timestamps are epoch 0 (offline).
    Returns reference IDs for assertions.
    """
    ts = 0
    db.execute(
